...
1. [Access & Identification of TPP](./fallback-pisp#access--identification-of-tpp)
   1. [Base URL](./fallback-pisp#base-url)
   2. [Characteristics of this solution](./fallback-pisp#characteristics-of-this-solution)
2. [User Authentication](./fallback-pisp#user-authentication)
   1. [Overview](./fallback-pisp#overview)
   2. [Step 1 - Start authentication with username & password](./fallback-pisp#step-1---start-authentication-with-username--password)
   3. [Step 2a - Recommended - Continue authentication with Two-Step Certification (OOB)](./fallback-pisp#step-2a---recommended---continue-authentication-with-two-step-certification-oob)
   4. [Step 2b - Not Recommended - Continue authentication with Two-Step SMS (OTP)](./fallback-pisp#step-2b---not-recommended---continue-authentication-with-two-step-sms-otp)
   5. [Step 3 - Get access token](./fallback-pisp#step-3---get-access-token)
3. [Payment Initiation](./fallback-pisp#payment-initiation)
   1. [Overview](./fallback-pisp#overview-1)
   2. [User PIN Encryption](./fallback-pisp#user-pin-encryption)
   3. [Initiate SEPA (Debit Transfer) DT Transactions](./fallback-pisp#initiate-sepa-debit-transfer-dt-transactions)
   4. [Initiate SEPA Standing Order (Recurring/Future Payments)](./fallback-pisp#initiate-sepa-standing-order-recurringfuture-payments)
   5. [Initiate SEPA Instant Transfer](./fallback-pisp#initiate-sepa-instant-transfer)
4. [Get Status of Initiated Transactions](./fallback-pisp#get-status-of-initiated-transactions)
   1. [Overview](./fallback-pisp#overview-2)
   2. [Get (Main) Account Transactions List](./fallback-pisp#get-main-account-transactions-list)
   3. [Get (Main) Account Transactions Details](./fallback-pisp#get-main-account-transactions-details)
   4. [Get Standing Order Transactions List](./fallback-pisp#get--standing-order-transactions-list)
   5. [Get (Main) Account Information](./fallback-pisp#get-main-account-information)
5. [Appendix A](./fallback-pisp#appendix-a)
...
> Please refer to the [Data which should not be stored](./fallback-pisp#data-which-should-not-be-stored-warning) section for more details.
...
Please make sure you’ve completed the steps from the [User PIN encryption](./fallback-pisp#user-pin-encryption) section before
proceeding with the
request.
